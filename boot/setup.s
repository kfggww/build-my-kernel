...
start:
    ...
    cli
    /*
	 * TODO: 8259重新编程. 8259是一种可编程中断控制芯片(PIC), 有目前应该有和8259
	 * 兼容的PIC, 由于IBM设计的问题, 导致在保护模式下8259的中断号和CPU的保留中断
	 * 冲突, 因此需要对8259重新编程, 以便把中断请求IRQ映射到不冲突的中断号上去.
	 * 现在出现了APIC, 高级中断控制器. 中断控制器的作用相当于一个中断的缓冲区.
	 */
